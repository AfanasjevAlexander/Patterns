...
class Originator:
    ...
    def __init__(self, state: str) -> None:
        self._state = state
        print(f"Oridinator: My inintial state is: {self._state}")

    def do_something(self) -> None:
        """
            Бизнес-логика Создателя может повлиять на его внутренне состояние.
        ...
        print("Ordinator: I'm going something important.")
        self._state = self._generate_random_string(30)
        print(f"Originator: and my state has changed to: {self._state}")

    @staticmethod
    def _generate_random_string(lenght: int = 10) -> str:
    ...
    def restore(self, memento: Memento) -> None:
        """
            Восстановливает состояние Создателя из объекта снимка.
        """
...
class Memento(ABC):
    """
        ...
        как дата создания или название. Однако он не расскрывает состояние Создателя.
    """
    @abstractmethod
...
if __name__ == "__main__":
    originator = Originator("Super-duper-super-puper_super")
