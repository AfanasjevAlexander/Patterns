...
/// 
/// It's going to be an output of `Visitors` trait which is defined for the type
/// in `visitor.rs`
#[derive(Default, Debug)]
...
/// of a desired type (in our case, `TwoValuesStruct` and `TwoValuesArray`)
...
fn main() {
    ...
    let result = deserializer.parse_str("123 465");
    println!("{:?}", result);
    ...
    let result = deserializer.parse_vec(vec![144, 4567]);
    println!("{:?}", result);

    println!("Error: {}", deserializer.parse_str("123 345").err().unwrap());
}
